...
//Credit : This file is an adapted version of ImGuiFileBrowser window from IcarusXIV. Source : https://github.com/IcarusXIV/Character-Select-/
    ...
    public class ImGuiFileBrowserWindow : Window
    {
        ...
        private void DrawQuickAccess()
        {
            ...
            foreach (var path in quickAccessPaths)
            {
                ...
                ImGui.PushFont(UiBuilder.IconFont);
                ImGui.TextColored(isSelected ? new Vector4(0.3f, 0.5f, 0.7f, 1f) : new Vector4(0.6f, 0.7f, 0.85f, 1f),
                    icon.ToIconString());
                ImGui.PopFont();
                ImGui.SameLine();
            ...
            if (pinnedPaths != null && pinnedPaths.Count > 0)
            {
                ...
                for (int i = 0; i < pinnedPaths.Count; i++)
                {
                    ...
                    ImGui.PushFont(UiBuilder.IconFont);
                    ImGui.TextColored(isSelected2 ? new Vector4(0.3f, 0.5f, 0.7f, 1f) : new Vector4(0.9f, 0.7f, 0.3f, 0.8f),
